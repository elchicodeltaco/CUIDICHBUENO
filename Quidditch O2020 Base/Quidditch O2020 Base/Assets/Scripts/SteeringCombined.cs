...
public class SteeringCombined : MonoBehaviour
{
    ...
    {
        seek = flee = arrive = pursuit = obstacleAvoidance = cohesion = wander = false;
    ...
    void Wander()
    {
        ...
        wanderTarget += new Vector3(Random.Range(-1f, 1f) * JitterThisTimeSlice,
        ...
        SteeringForce += newTarget * wanderWeight;
    }

    void Wander2()
    {

        // Elegimos una direccion aleatoria
        wanderTarget += new Vector3(
            Random.Range(-1f, 1f), Random.Range(-1f, 1f), Random.Range(-1f, 1f));

        // Normalizamos para llevar el punto a una circunferencia
        wanderTarget.Normalize();

        //WanderTarget = Random.insideUnitCircle.normalized;

        // escalamos de acuerdo al radio de wander
        wanderTarget *= wanderRadius;

        // mover el objetivo a una posicion frente al agente
        // a una distancia "wanderDistance"
        Vector3 newTarget = transform.forward * wanderDistance;
        newTarget += wanderTarget;
    ...
    void Interpose()
    {
        ...
        Vector3 agent1Vel = agent1.GetComponent<SteeringCombined>() ? agent1.GetComponent<SteeringCombined>().Velocity : Vector3.zero;
        Vector3 agent2Vel = agent2.GetComponent<SteeringCombined>() ? agent2.GetComponent<SteeringCombined>().Velocity : Vector3.zero;
        Vector3 Apos = agent1.transform.position + agent1Vel * TimeToReachMidPoint;
        Vector3 Bpos = agent2.transform.position + agent2Vel * TimeToReachMidPoint;
        ...
        if (distance > 0)
        {
            ...
            SteeringForce += (DesiredVelocity - Velocity) * interposeWeight;
            return;
        }
